...
class Program
{
    ...
    static void Main()
    {
        ...
        {
            int age = today.Year - d.BirthDay.Year;
            if (d.BirthDay > today.AddYears(-age)) age--;
    ...
    .Select(d => new
    {
        Surname = d.FullName.Split(' ').Last(),
        Age = today.Year - d.BirthDay.Year - (d.BirthDay > today.AddYears(-(today.Year - d.BirthDay.Year)) ? 1 : 0),
        d.Debt
    }))
    ...
    .Where(d => d.Phone.Where(char.IsDigit).GroupBy(c => c).All(g => g.Count() == 1)))
        ...
        foreach (var d in debtors.Where(d => {
            ...
            int months = ((nextBday.Year - today.Year) * 12 + nextBday.Month - today.Month);
